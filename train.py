...
# Load data
print("Loading data...")
x_text,x_num, y = data_helpers.load_data_labels(codeDescription = 'Travel')
...
x_text_shuffled = x[shuffle_indices]
y_shuffled = y[shuffle_indices]
x_num_shuffled = x_num[shuffle_indices]

# Split train/test set
# TODO: This is very crude, should use cross-validation
x_text_train, x_num_train,x_text_dev,x_num_dev = x_text_shuffled[:-1000], x_num_shuffled[:-1000],x_text_shuffled[-1000:],x_num_shuffled[-1000:]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            sequence_length=x_text_train.shape[1],
            num_classes=2,
            vocab_size=len(vocab_processor.vocabulary_),
            embedding_size=FLAGS.embedding_dim,
            filter_sizes=list(map(int, FLAGS.filter_sizes.split(","))),
            num_filters=FLAGS.num_filters,num_features = x_num.shape[1],
        ...
        def train_step(x_text_train,x_num_train, y_batch):
            """
        ...
        batches = data_helpers.batch_iter(
            list(zip(x_text_train,x_num_train,y_train)), FLAGS.batch_size, FLAGS.num_epochs)
        # Training loop. For each batch...
        for batch in batches:
            x_text_batch, x_num_batch,y_batch = zip(*batch)
            train_step(x_text_batch,x_num_batch,y_batch)
